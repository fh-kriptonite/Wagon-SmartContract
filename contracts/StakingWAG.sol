...
contract StakingWAG is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function initialize(address _stakingToken, address _rewardToken) public initializer {
        ...
        __ERC20_init("Staked WAG", "StkWAG");
    ...
    function addRewardCycle(
        uint _amount, uint _duration
    ) external nonReentrant onlyRole(ADMIN_ROLE) updateReward(address(0)) {
        uint _finishAt = block.timestamp + _duration;
        updateRewardRate(_amount, _finishAt);
