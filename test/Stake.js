...
// ------------------------------------------------------------------------------------------------------------------------ //
  ...
  async function InjectRewardPrestartFixture() {
    ...
    const delay = 50;
    const duration = 100;
    const startAt = currentTimestamp + delay;
    const finishAt = currentTimestamp + delay + duration;

    await stake.initialRewardCycle("200000000000000000000", startAt, finishAt);
  ...
  async function AddRewardCycleFixture() {
    ...
    const currentTimestamp = (await ethers.provider.getBlock()).timestamp;
    const duration = 100;
    const finishAt = currentTimestamp + duration + 1;

    await stake.addRewardCycle("100000000000000000000", finishAt);
